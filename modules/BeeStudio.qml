...
Item {
    ...
    // ─── Catégorie active ────────────────────────────────────────
    property int activeCategory: 0   // 0=Dashboard 1=Alvéoles 2=Fonds d'écran 3=Historique 4=Presets 5=Profils
    ...
    property bool   _cellDeletable:   true   // based on model's customizable flag
    ...
    function loadCell(index) {
        ...
        selectedIndex    = index
        editCustomizable = true  // Always allow editing
        _cellDeletable = c.customizable !== false  // Only prevent deletion of protected cells
        editIcon         = c.icon        || ""
        editTitle        = c.title       || ""
    ...
    Component.onCompleted: {
        if (beeStudio.embedded) {
            // In embedded mode: no animations, content is directly visible
            studioPanel.opacity = 1.0
            studioPanel.scale = 1.0
        } else {
            backdropIn.start()
            openAnim.start()
        }
    ...
    function requestClose() {
        if (beeStudio.embedded) {
            // In embedded mode, just hide immediately
            beeStudio.visible = false
        } else {
            backdropOut.start()
            closeAnim.start()
        }
    ...
    Rectangle {
        ...
        // Skip backdrop animation in embedded mode — already visible
        Connections {
            target: beeStudio
            function onEmbeddedChanged() {
                if (beeStudio.embedded) backdrop.opacity = 0
            }
        }
    ...
    Rectangle {
        id: studioPanel
        // ─── Embedded mode: flatten the panel ────────────────
        // When embedded, the panel becomes transparent, borderless,
        // and fills the parent directly — no nested window frame.
        anchors.fill: beeStudio.embedded ? parent : undefined
        anchors.centerIn: beeStudio.embedded ? undefined : parent
        width:  beeStudio.embedded ? undefined : Math.min(parent.width  * 0.90, 1020)
        height: beeStudio.embedded ? undefined : Math.min(parent.height * 0.86, 700)
        radius: beeStudio.embedded ? 0 : 20
        color: beeStudio.embedded
            ? (BeeTheme.mode === "HoneyDark"
                ? Qt.rgba(0.06, 0.05, 0.08, 1.0)
                : Qt.rgba(0.97, 0.95, 0.90, 1.0))
            : (BeeTheme.mode === "HoneyDark"
                ? Qt.rgba(0.05, 0.05, 0.07, 0.97)
                : Qt.rgba(0.97, 0.95, 0.90, 0.97))
        border.color: beeStudio.embedded
            ? "transparent"
            : Qt.rgba(BeeTheme.accent.r, BeeTheme.accent.g, BeeTheme.accent.b, 0.35)
        border.width: beeStudio.embedded ? 0 : 1.5
        clip: true
        opacity: beeStudio.embedded ? 1.0 : 0
        scale:  beeStudio.embedded ? 1.0 : 0.90

        Behavior on color        { ColorAnimation { duration: 600 } }
        Behavior on border.color { ColorAnimation { duration: 600 } }

        // Background Sidebar intégré pour éviter la coupure de la ligne
        // Hidden in embedded mode (sidebar is already hidden)
        Rectangle {
            visible: !beeStudio.embedded
        ...
        // Subtle aura glow on border — disabled in embedded mode
        layer.enabled: !beeStudio.embedded
            ...
            Item {
                ...
                ColumnLayout {
                    ...
                    ListModel {
                        ...
                        ListElement { catIcon: "👥"; catKey: "category_profiles"; catSub: "profiles_desc" }
                    }
            ...
            Item {
                    ...
                    ColumnLayout {
                        ...
                        Item {
                            ...
                            RowLayout {
                                ...
                                // Delete button (only when a cell is selected and is deletable)
                                Rectangle {
                                    width: 100; height: 30; radius: 15
                                    color: beeStudio.selectedIndex >= 0 && beeStudio._cellDeletable ? Qt.rgba(0.9, 0.2, 0.2, 0.15) : Qt.rgba(0.5, 0.5, 0.5, 0.1)
                                    border.color: beeStudio.selectedIndex >= 0 && beeStudio._cellDeletable ? "#ff4444" : "#888888"
                                    border.width: 1
                                    opacity: beeStudio.selectedIndex >= 0 && beeStudio._cellDeletable ? 1 : 0.5
                                    Text { 
                                        text: "🗑️ Delete"; 
                                    ...
                                    MouseArea { 
                                        anchors.fill: parent; cursorShape: Qt.PointingHandCursor
                                        enabled: beeStudio.selectedIndex >= 0 && beeStudio._cellDeletable
                                        onClicked: {
                                            if (beeStudio.selectedIndex >= 0 && beeStudio._cellDeletable) {
                ...
                Item {
                    ...
                    ColumnLayout {
                        ...
                        // ── Originaux (statiques, toujours visibles) ──
                        ColumnLayout {
                            Layout.fillWidth: true
                            spacing: 12
                            Text {
                        ...
                        // ── Bibliothèque (défilable avec scrollbar) ──
                        ColumnLayout {
                            Layout.fillWidth: true; Layout.fillHeight: true
                            spacing: 12
                            Text {
                            ...
                            Flickable {
                                Layout.fillWidth: true; Layout.fillHeight: true
                                contentHeight: libraryGrid.height + 20
                                clip: true
                                ScrollBar.vertical: ScrollBar {
                                    width: 6; policy: ScrollBar.AsNeeded
                                    contentItem: Rectangle { radius: 3; color: Qt.rgba(BeeTheme.accent.r, BeeTheme.accent.g, BeeTheme.accent.b, 0.3) }
                                }
                ...
                Item {
                    id: panelPresets
                    anchors.fill: parent
                    visible: beeStudio.activeCategory === 4 || beeStudio.activeCategory === 5
                            ...
                            Flickable {
                                ...
                                ColumnLayout {
                                    ...
                                    Rectangle {
                                        ...
                                        border.color: BeePresets.hasAutoSave() ? Qt.rgba(BeeTheme.accent.r, BeeTheme.accent.g, BeeTheme.accent.b, 0.30) : Qt.rgba(BeeTheme.textPrimary.r, BeeTheme.textPrimary.g, BeeTheme.textPrimary.b, 0.10)
                                        border.width: 1
                                        opacity: BeePresets.hasAutoSave() ? 1.0 : 0.4
                                        RowLayout {
                                            anchors.centerIn: parent
                                            spacing: 6
                                        ...
                                        MouseArea {
                                            id: restHov
                                            anchors.fill: parent
                                            cursorShape: BeePresets.hasAutoSave() ? Qt.PointingHandCursor : Qt.ArrowCursor
                                            hoverEnabled: true
                                            onClicked: {
                                                if (BeePresets.hasAutoSave()) {
                                                    BeePresets.restoreAutoSave()
                                                }
                                            }
                                        }
                                        ToolTip.visible: restHov.containsMouse && !BeePresets.hasAutoSave()
                                        ToolTip.text: BeeConfig.uiLang === "fr" ? "Aucune sauvegarde disponible" : "No backup available"
                                        ToolTip.delay: 500
                                    }
                                    ...
                                    Flow {
                                        ...
                                        Repeater {
                                            model: BeePresets.presets.filter(function(p) { return p.name.charAt(0) !== "_" })
                    ...
                    // ═══════════════════════════════════════════════════════
                    // PANNEAU 5 : PROFILS 👥  (Profile Switching Multi-User)
                    // ═══════════════════════════════════════════════════════
                    Item {
                        id: panelProfiles
                        anchors.fill: parent
                        visible: beeStudio.activeCategory === 5
                                ...
                                RowLayout {
                                    ...
                                    Rectangle {
                                        ...
                                        RowLayout {
                                            ...
                                            spacing: 5
                                            Text {
                                                text: BeeProfiles.currentProfile.icon || "🍯"
                                                font.pixelSize: 13
                                            }
                                            Text {
                                        ...
                                        Flow {
                                            ...
                                            Repeater {
                                                ...
                                                delegate: Rectangle {
                                                    ...
                                                    border.color: isActiveProfile
                                                        ? BeeTheme.accent
                                                        : Qt.rgba(BeeTheme.accent.r, BeeTheme.accent.g, BeeTheme.accent.b, 0.18)
                                                    border.width: isActiveProfile ? 2 : 1
                                                    Behavior on color { ColorAnimation { duration: 150 } }
                                                    Behavior on border.color { ColorAnimation { duration: 200 } }
                                                    Behavior on border.width { NumberAnimation { duration: 200 } }

                                                    property bool isActiveProfile: modelData.id === BeeProfiles.activeProfileId

                                                    ColumnLayout {
                                                        anchors { fill: parent; margins: 16; topMargin: 18 }
                                                        spacing: 8

                                                        // Profile icon + active indicator
                                                        Item {
                                                            ...
                                                            // Active badge (golden ring)
                                                            Rectangle {
                                                                id: activeBadge
                                                                visible: parent.parent.isActiveProfile
                                                                anchors.centerIn: parent
                                                                width: 52; height: 52
                                                                radius: 26
                                                                color: "transparent"
                                                                ...
                                                                SequentialAnimation on border.color {
                                                                    loops: Animation.Infinite
                                                    running: activeBadge.visible
                                                                    ColorAnimation { to: Qt.rgba(BeeTheme.accent.r, BeeTheme.accent.g, BeeTheme.accent.b, 0.5); duration: 1200; easing.type: Easing.InOutSine }
                                                                    ColorAnimation { to: BeeTheme.accent; duration: 1200; easing.type: Easing.InOutSine }
                                                                }
                                                            }
                                                        }
                                                        ...
                                                        // Theme indicator
                                                        Text {
                                                            text: modelData.activeTheme === "HoneyLight" ? "☀️ Light" : "🌙 Dark"
                                                            color: Qt.rgba(BeeTheme.textPrimary.r, BeeTheme.textPrimary.g, BeeTheme.textPrimary.b, 0.35)
                                                            font.pixelSize: 10
                                                            Layout.alignment: Qt.AlignHCenter
                                                        }
                                                        ...
                                                        Text {
                                                            ...
                                                            MouseArea {
                                                                ...
                                                                onExited: parent.color = Qt.rgba(BeeTheme.textPrimary.r, BeeTheme.textPrimary.g, BeeTheme.textPrimary.b, 0.30)
                                                                onClicked: {
                                                                    BeeProfiles.deleteProfile(modelData.id)
                                                                }
                                                            }
                                                        }
                                                    }
                                                    ...
                                                    MouseArea {
                                                        ...
                                                        cursorShape: Qt.PointingHandCursor; hoverEnabled: true
                                                        onClicked: {
                                                            if (!parent.isActiveProfile && !BeeProfiles.switching) {
                                                                BeeProfiles.switchProfile(modelData.id)
                                                            }
                                                        }
                                                    }
                                                }
                                            ...
                                            Rectangle {
                                                width: 180; height: 180
                                                radius: 14
                                                color: addProfHov.containsMouse
                    ...
                    // Cross-fade overlay during profile switch (300ms in + 300ms out)
                    Rectangle {
                        id: profileTransitionOverlay
                        anchors.fill: parent
                        color: BeeTheme.mode === "HoneyDark" ? Qt.rgba(0.06, 0.06, 0.08, 1) : Qt.rgba(0.97, 0.95, 0.90, 1)
                        opacity: BeeProfiles.transitionOpacity
                        visible: BeeProfiles.transitionOpacity > 0.01
                        z: 100
    ...
    Popup {
        ...
        background: Rectangle {
            color: BeeTheme.mode === "HoneyDark" ? Qt.rgba(0.10, 0.10, 0.14, 0.98) : Qt.rgba(0.97, 0.95, 0.90, 0.98)
            radius: 16
            border.color: Qt.rgba(BeeTheme.accent.r, BeeTheme.accent.g, BeeTheme.accent.b, 0.30)
            border.width: 1
        }
        ...
        ColumnLayout {
            ...
            TextField {
                ...
                Layout.fillWidth: true
                height: 36
                placeholderText: tr("profiles_name_placeholder")
                color: BeeTheme.textPrimary
                font.pixelSize: 13
                leftPadding: 10; rightPadding: 10
                background: Rectangle {
                    radius: 7
                    color: Qt.rgba(BeeTheme.accent.r, BeeTheme.accent.g, BeeTheme.accent.b, 0.07)
                    border.color: Qt.rgba(BeeTheme.accent.r, BeeTheme.accent.g, BeeTheme.accent.b, newProfileName.activeFocus ? 0.5 : 0.15)
                    border.width: 1
                    Behavior on border.color { ColorAnimation { duration: 150 } }
                }
            }
            ...
            RowLayout {
                ...
                Repeater {
                    ...
                    delegate: Rectangle {
                        ...
                        color: newProfileIcon === modelData
                            ? Qt.rgba(BeeTheme.accent.r, BeeTheme.accent.g, BeeTheme.accent.b, 0.20)
                            : Qt.rgba(BeeTheme.accent.r, BeeTheme.accent.g, BeeTheme.accent.b, 0.04)
                        border.color: newProfileIcon === modelData ? BeeTheme.accent : "transparent"
                        border.width: 1
                        ...
                        MouseArea {
                            anchors.fill: parent
                            cursorShape: Qt.PointingHandCursor
                            onClicked: newProfileIcon = modelData
                        }
                    }
                }
            }
            ...
            Rectangle {
                ...
                MouseArea {
                    ...
                    onClicked: {
                        ...
                        if (name.length > 0) {
                            BeeProfiles.createProfile(name, newProfileIcon)
                            newProfileName.text = ""
                            newProfileIcon = "🍯"
